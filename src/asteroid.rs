...
impl<'a>System<'a> for AsteroidMover {
  type SystemData = (
    ...
    WriteStorage<'a, crate::components::Asteroid>
  );
  ...
  fn run(&mut self, mut data: Self::SystemData) {
    for (pos, rend, asteroid) in (&mut data.0, &mut data.1, &data.2).join() {
      ...
      if pos.y > (crate::GAME_HEIGHT - half_height).into()
      ...
      if rend.rot > 360.0 { rend.rot -= 360.0 }
      if rend.rot < 0.0 { rend.rot += 360.0 }
    }
  }
}
...
impl<'a>System<'a> for AsteroidCollider {
  type SystemData = (
    WriteStorage<'a, crate::components::Position>,
    WriteStorage<'a, crate::components::Renderable>,
    WriteStorage<'a, crate::components::Player>,
    WriteStorage<'a, crate::components::Asteroid>,
    Entities<'a>
  );
